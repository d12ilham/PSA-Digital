...
export default function ReportEditPage({
  ...
  const [landingIntroDesc, setLandingIntroDesc] = useState("");
  const [landingIntroBullets, setLandingIntroBullets] = useState("");
  const [landingExecDesc, setLandingExecDesc] = useState("");
  const [landingExecBullets, setLandingExecBullets] = useState("");
  ...
  const [coverImageMode, setCoverImageMode] = useState<
    "url" | "upload" | "library"
  >("library");
  const [pdfMode, setPdfMode] = useState<"url" | "upload" | "library">(
    "library",
  );
  ...
  const loadPageData = async () => {
    ...
    try {
      ...
      setLandingIntroDesc(reportData.landingIntroDesc || "");
      setLandingIntroBullets(reportData.landingIntroBullets || "");
      setLandingExecDesc(reportData.landingExecDesc || "");
      setLandingExecBullets(reportData.landingExecBullets || "");
  ...
  const handleSaveReport = async (e: React.FormEvent) => {
    ...
    try {
      const updated = await api.patch<Report>(`/reports/${report.id}`, {
        ...
        isFeatured,
        status,
        landingIntroDesc: landingIntroDesc || null,
        landingIntroBullets: landingIntroBullets || null,
        landingExecDesc: landingExecDesc || null,
        landingExecBullets: landingExecBullets || null,
      });
  ...
  const handleInitializeEmptyPages = async () => {
    ...
    if (
      !confirm(
        "Do you want to initialize default structural pages/chapters for this dataset?",
      )
    )
      return;
    try {
      ...
      setActiveReport(reportData);
      toast.success(
        "Default report structure has been successfully initialized.",
      );
    } catch (err: any) {
      toast.error(err);
    } finally {
  ...
  const chapters = [
    {
      num: "01",
      title: "About PSA & Methodology",
      type: "about",
      desc: "Introduction details, sector overview guidelines, and dataset methodology.",
    },
    {
      num: "02",
      title: "Executive Summary",
      type: "executive_summary",
      desc: "High-level insights, summary cards, and transition dashboard landing blocks.",
    },
    {
      num: "03",
      title: "Drivers of Change",
      type: "drivers_of_change",
      desc: "Key societal, industrial, and economic vectors shaping local workforce strategies.",
    },
    {
      num: "04",
      ...
      nested: [
        {
          ...
          desc: "National-level segment parameters and main takeaways.",
        },
        {
          title: "State & Territory Profile",
          type: "state_territory",
          desc: "Detailed state-specific demographic breakdowns.",
        },
        {
          title: "Industry Profile",
          type: "industry_profile",
          desc: "Socio-economic parameters and workforce characteristics.",
        },
      ],
    },
    {
      ...
      type: "workforce_insights",
      desc: "Dynamic indicators, employment statistics charts, and database grids.",
    },
    {
      num: "06",
      title: "Workforce Strategies",
      type: "strategies",
      desc: "Roadmaps, strategy action lists, and recommendations blocks.",
    },
    {
      num: "07",
      title: "Looking Forward",
      type: "looking_forward",
      desc: "Concluding remarks, future projections, and summary contact panels.",
    },
  ];
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div className="mb-1 flex items-center gap-2 font-mono text-xs uppercase text-muted">
            ...
            <span>/</span>
            <span
              className="cursor-pointer hover:underline text-muted"
              onClick={() => router.push("/dashboard/reports")}
            >
              Reports
            </span>
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="wireframe-badge uppercase font-bold text-xs">
            {report.status}
          </span>
        </div>
      </div>
            ...
            <div className="bg-[#252D02] px-6 py-4 flex items-center justify-between text-[#FAFAF0]">
              ...
              {(user?.role === "admin" || user?.role === "editor") && (
                <button
                  ...
                  className="flex items-center gap-1.5 text-xs font-sans font-bold bg-[#8AC900] text-[#252D02] hover:bg-[#416102] hover:text-white px-3 py-1.5 rounded-lg transition-all cursor-pointer border border-[#252D02]"
                >
              ...
              {pages.length === 0 ? (
                <div className="text-center py-10 border border-dashed border-border/80 bg-sidebar/10 p-6 rounded">
                  ...
                  <p className="text-xs text-muted mb-4">
                    This report has no structural chapters initialized yet.
                    Seeding default chapters is required to edit content.
                  </p>
                  <button
                            ...
                            {subs.length > 0 && (
                              <div className="mt-3 pl-8 ml-3 border-l-2 border-border/40 space-y-3">
                                {subs.map((sub) => (
                                  <div
                                    key={sub.id}
                                    className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 py-1 first:pt-0 last:pb-0"
                                  >
                                    <div>
            ...
            <div className="p-6">
              ...
              {activeSubTab === "details" && (
                <form onSubmit={handleSaveReport} className="space-y-4">
                  {message && (
                    <div className="border border-green-200 bg-green-50/50 p-2.5 text-xs text-green-700 font-mono">
                      * {message.toUpperCase()}
                    </div>
                  )}
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-0">
                          <button
                            ...
                            className={`px-2 py-0.5 font-mono text-xs uppercase border-y border-l border-border transition-colors ${
                          ...
                          <button
                            ...
                            className={`px-2 py-0.5 font-mono text-xs uppercase border-y border-x border-border transition-colors ${
                              coverImageMode === "url"
                                ? "bg-primary text-white border-primary"
                                : "bg-card text-muted hover:bg-sidebar"
                            }`}
                          ...
                          <button
                            ...
                            className={`px-2 py-0.5 font-mono text-xs uppercase border border-border transition-colors ${
                        ...
                        <div className="space-y-1.5">
                          ...
                          {pdfFileUrl && (
                            <div className="flex items-center gap-2 border border-border bg-sidebar/20 p-2">
                              ...
                              <p className="font-mono text-xs text-muted truncate flex-1">
                                {pdfFileUrl}
                              </p>
                              <button
                                type="button"
                      ...
                      {pdfMode === "upload" && (
                        <div className="space-y-1.5">
                          ...
                          >
                            ...
                            <span className="font-mono text-xs text-muted/50">
                              PDF only · Max{" "}
                              {process.env.NEXT_PUBLIC_MAX_FILE_MB ?? 20}MB
                            </span>
                          </button>
                          {pdfFileUrl && (
                            <div className="flex items-center gap-2 border border-border bg-sidebar/20 p-2">
                    ...
                    <div className="border-t border-border/60 pt-4 space-y-4">
                      ...
                      <div className="space-y-3 p-3 bg-sidebar/10 border border-border/80 rounded">
                        ...
                        <div className="space-y-1">
                          ...
                          <textarea
                            rows={2}
                            value={landingIntroDesc}
                            onChange={(e) =>
                              setLandingIntroDesc(e.target.value)
                            }
                            className="w-full border border-border bg-card px-2.5 py-1.5 text-xs text-primary focus:outline-none placeholder:text-muted/30"
                            placeholder="Default preview text will be used if left empty..."
                          />
                        </div>
                        <div className="space-y-1">
                          <label className="block font-mono text-xs uppercase text-muted">
                          ...
                          <textarea
                            rows={3}
                            value={landingIntroBullets}
                            onChange={(e) =>
                              setLandingIntroBullets(e.target.value)
                            }
                            className="w-full border border-border bg-card px-2.5 py-1.5 text-xs text-primary focus:outline-none placeholder:text-muted/30 font-sans"
                            placeholder="Default bullets will be used if left empty..."
                          />
                        </div>
                      </div>
                      ...
                      <div className="space-y-3 p-3 bg-sidebar/10 border border-border/80 rounded">
                        ...
                        <div className="space-y-1">
                          ...
                          <textarea
                            rows={2}
                            value={landingExecDesc}
                            onChange={(e) => setLandingExecDesc(e.target.value)}
                            className="w-full border border-border bg-card px-2.5 py-1.5 text-xs text-primary focus:outline-none placeholder:text-muted/30"
                            placeholder="Default preview text will be used if left empty..."
                          />
                        </div>
                        <div className="space-y-1">
                          ...
                          <textarea
                            rows={3}
                            value={landingExecBullets}
                            onChange={(e) =>
                              setLandingExecBullets(e.target.value)
                            }
                            className="w-full border border-border bg-card px-2.5 py-1.5 text-xs text-primary focus:outline-none placeholder:text-muted/30 font-sans"
                            placeholder="Default bullets will be used if left empty..."
                          />
                    ...
                    <div className="relative">
                      ...
                      {showInitTooltip && (
                        <div className="absolute bottom-full right-0 mb-2 w-64 bg-primary text-white text-xs font-mono leading-relaxed p-3 z-50">
                          ...
                          <p>
                            Creates the default chapter pages for this report
                            (e.g. About, Executive Summary, Drivers of Change,
                            Industry Overview, Workforce Insights, Strategies,
                            Looking Forward).
                          </p>
                          <p className="mt-1 text-white/70">
                            If chapters already exist, this will only create any
                            missing ones — it will NOT overwrite existing
                            content.
                          </p>
              ...
              {activeSubTab === "kpis" && (
                <div className="space-y-4">
                  <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
                    {kpis.length === 0 ? (
                      <div className="text-xs text-muted/60 font-mono py-4 italic border border-dashed border-border p-3 bg-sidebar/20 text-center">
              ...
              {activeSubTab === "history" && (
                <div className="space-y-3">
                  {logsLoading ? (
                    ...
                    <div className="text-center py-8 font-mono text-xs uppercase text-muted italic border border-dashed border-border p-3 bg-sidebar/20">
                      No history found.
                    </div>
                  ) : (
