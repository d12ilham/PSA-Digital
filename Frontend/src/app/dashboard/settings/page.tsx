...
export default function SettingsPage() {
  ...
  const handleDeleteTemplate = async (id: string, name: string) => {
    if (
      !confirm(
        `Are you sure you want to delete the chapter template "${name}"?`,
      )
    )
      return;
    setTemplatesError(null);
  ...
  const handleSeedDefaults = async () => {
    if (
      !confirm(
        "Do you want to seed the database with the default 9 report chapters?",
      )
    )
      return;
    setTemplatesLoading(true);
    ...
    const defaultPages = [
      ...
      },
      {
        ...
        pageType: "drivers_of_change",
        slug: "drivers-of-change",
        sortOrder: 3,
      },
      {
        title: "Sector Overview",
        pageType: "industry_overview",
        slug: "sector-overview",
        sortOrder: 4,
        parentPathway: "executive_summary",
      },
      {
        title: "State & Territory Profile",
        pageType: "state_territory",
        slug: "state-territory-profile",
        sortOrder: 5,
        parentPathway: "executive_summary",
      },
      {
        title: "Industry Profile",
        pageType: "industry_profile",
      ...
      },
      {
        ...
        pageType: "strategies",
        slug: "workforce-strategies",
        sortOrder: 8,
      },
      {
        title: "Looking Forward",
        pageType: "looking_forward",
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-8 lg:grid-cols-2">
        ...
        <div className="space-y-8">
          ...
          <div className="border border-border bg-card p-6 rounded-2xl relative">
            ...
            {!isEditorOrAdmin ? (
              <div className="text-xs text-muted/60 font-mono py-6 italic border border-dashed border-border p-4 bg-sidebar/20 text-center">
                * Access Denied: Only Admins or Editors can register new
                industry segments.
              </div>
            ) : (
          ...
          <div className="border border-border bg-card p-6 rounded-2xl relative">
            ...
            {!isEditorOrAdmin ? (
              <div className="text-xs text-muted/60 font-mono py-6 italic border border-dashed border-border p-4 bg-sidebar/20 text-center">
                * Access Denied: Only Admins or Editors can register new report
                years.
              </div>
            ) : (
      ...
      <div className="border border-border bg-card p-6 rounded-2xl relative">
        ...
        {!isEditorOrAdmin ? (
          <div className="text-xs text-muted/60 font-mono py-6 italic border border-dashed border-border p-4 bg-sidebar/20 text-center">
            * Access Denied: Only Admins or Editors can manage report chapter
            templates.
          </div>
        ) : (
          <div className="space-y-6">
            ...
            <div className="grid gap-8 lg:grid-cols-3">
              ...
              <div className="lg:col-span-1 border-r border-border/60 pr-8 space-y-4">
                ...
                <form
                  onSubmit={
                    editingTplId ? handleUpdateTemplate : handleAddTemplate
                  }
                  className="space-y-3"
                >
                  <div className="space-y-1">
                  ...
                  <div className="space-y-1">
                    ...
                    <input
                      ...
                      className="w-full border border-border bg-[#fdfdfc] px-3 py-1.5 text-xs text-primary focus:border-primary focus:outline-none font-mono text-xs"
                    />
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    >
                      ...
                      <option value="drivers_of_change">
                        Drivers of Change
                      </option>
                      <option value="industry_overview">
                        Industry Overview
                      </option>
                      <option value="state_territory">
                        State & Territory Profile
                      </option>
                      <option value="industry_profile">Industry Profile</option>
                      <option value="workforce_insights">
                        Workforce Insights
                      </option>
                      <option value="strategies">Strategies</option>
                      <option value="strategy_update">Strategy Update</option>
                      <option value="existing_strategies">
                        Existing Strategies
                      </option>
                      <option value="federal_initiatives">
                        Federal Initiatives
                      </option>
                  ...
                  <div className="space-y-1">
                    ...
                    <select
                      value={
                        editingTplId ? editParentPathway : newParentPathway
                      }
                      onChange={(e) =>
                        editingTplId
                          ? setEditParentPathway(e.target.value)
                          : setNewParentPathway(e.target.value)
                      }
                      className="w-full border border-border bg-card px-2.5 py-1.5 text-xs text-primary focus:outline-none"
                    >
                      ...
                      {templates
                        .filter(
                          (t) => !t.parentPathway && t.id !== editingTplId,
                        )
                        .map((t) => (
                          <option key={t.id} value={t.pageType}>
                            {t.title}
                          </option>
                        ))}
                    </select>
                    <p className="font-mono text-xs text-muted/65 leading-relaxed mt-0.5">
                      Nest this chapter inside another parent (e.g. Industry
                      Overview segments).
                    </p>
                  </div>
                ...
                {templatesLoading ? (
                  ...
                  <div className="text-center py-10 border border-dashed border-border bg-sidebar/10 p-6">
                    <p className="text-xs text-muted mb-2">
                      No templates are defined in the database.
                    </p>
                    <p className="text-xs text-muted/60 leading-relaxed max-w-sm mx-auto">
                      Click the "Seed Default Chapters" button above to populate
                      standard chapters (About, Executive Summary, etc.)
                      automatically.
                    </p>
                  </div>
                ) : (
                    ...
                    return (
                      <div className="border border-border divide-y divide-border/60 bg-[#fafafa]/30">
                        {topLevel.map((tpl, idx) => {
                          ...
                          const globalIdx = templates.findIndex(
                            (t) => t.id === tpl.id,
                          );
                          ...
                          return (
                            <div
                              key={tpl.id}
                              className="p-3.5 hover:bg-sidebar/5 transition-colors"
                            >
                              <div className="flex items-center justify-between gap-4">
                                <div className="flex items-center gap-3">
                                  ...
                                  <div>
                                    <h4 className="text-xs font-bold text-primary flex items-center gap-2 leading-none">
                                      {tpl.title}
                                      <span className="font-mono text-xs uppercase font-normal text-muted/60 border border-border px-1 py-0.5 bg-card">
                                        Type: {tpl.pageType}
                                      </span>
                                    </h4>
                                    <p className="font-mono text-xs text-muted mt-1">
                                      Slug: {tpl.slug} | Sort: {tpl.sortOrder}
                                    </p>
                                  </div>
                                </div>
                                ...
                                <div className="flex items-center gap-1 shrink-0">
                                  <button
                                    ...
                                    onClick={() =>
                                      handleMoveTemplate(globalIdx, "up")
                                    }
                                  ...
                                  <button
                                    type="button"
                                    disabled={
                                      globalIdx === templates.length - 1
                                    }
                                    onClick={() =>
                                      handleMoveTemplate(globalIdx, "down")
                                    }
                                    className="p-1 border border-border hover:bg-sidebar text-muted hover:text-primary transition-colors disabled:opacity-30 cursor-pointer"
                                    title="Move Down"
                                  >
                              ...
                              {subs.length > 0 && (
                                <div className="mt-3.5 pl-6 border-l-2 border-border/40 space-y-2.5">
                                  {subs.map((sub, sIdx) => {
                                    const subGlobalIdx = templates.findIndex(
                                      (t) => t.id === sub.id,
                                    );
                                    ...
                                    return (
                                      <div
                                        key={sub.id}
                                        className="flex items-center justify-between gap-4 py-1.5 border-b border-border/30 last:border-0 hover:bg-sidebar/10 p-2"
                                      >
                                        <div>
                                          <h5 className="text-xs font-bold text-primary flex items-center gap-1.5 leading-none">
                                            {sub.title}
                                            <span className="font-mono text-xs uppercase font-normal text-muted/65 border border-border/60 px-1 py-0.5 bg-card">
                                              Type: {sub.pageType}
                                            </span>
                                          </h5>
                                          <p className="font-mono text-xs text-muted mt-1">
